...
function checarCpf(cpf) {
  ...
  if (/^(\d)\1+$/.test(cpfLimpo))
    erro("CPF inválido: todos os dígitos são iguais", 400);
  let check = cpfLimpo.slice(0, 9).split("").map(Number);
  function calcularDigito(entrada, pesoInicial) {
    let valor =
      entrada.reduce((ac, x, i) => ac + x * (pesoInicial - i), 0) % 11;
...
function checarData(data) {
  ...
  if (dataNascimento > hoje)
    erro("A data de nascimento não pode ser superior ao dia atual", 400);
}

function checarMedico(funcao, crm) {
  if (funcao === "Medico" && !crm)
    erro("CRM é obrigatório para cadastrar médico", 400);
  if (funcao !== "Medico" && crm)
    erro("CRM é um campo exclusivo para médicos", 400);
}
...
>>>>>>> 6c0822d5b44bfa10d781bb3c041b1143e3a7b5f8
  ...
  static async login(dados) {
    ...
    if (pessoa.status !== "Ativo")
      erro(
        "Seu cadastro está inativo, entre em contato com um funcionário ou administrador do site",
        403,
      );
  ...
  static async criar(dados, funcaoReq) {
    ...
    const faltando = funcoes[funcao].obrigatorios.filter(
      (campo) =>
    ...
    if (faltando.length)
      erro(`Há campos obrigatórios em branco: ${faltando.join(", ")}`, 400);

    // checagem permissão
    console.log("Função requisitante:", funcaoReq);
    console.log("Função que está tentando criar:", funcao);
    console.log("Funções que pode gerenciar:", funcoes[funcaoReq]?.gerenciar);
    console.log(
      "Permitido?",
      (funcoes[funcaoReq]?.gerenciar || []).includes(funcao),
    );
  ...
  static async atualizar(id, dados, requisitante) {
    const { cpf, nome, data_nascimento, telefone, endereco, senha, crm } =
      dados;
    ...
    const permitido =
      id == requisitante.id ||
      ((funcoes[requisitante.funcao] || {}).gerenciar || []).includes(
        pessoa.funcao,
      );
    if (!permitido) erro("Acesso negado", 403);
    ...
    if (pessoaCpf && pessoaCpf.id != id)
      erro("CPF cadastrado em outra pessoa", 409);

    // checagem tentativa mudar função
    // console.log(dados.funcao);
    // if (dados.funcao) erro("Tentativa de mudança de função foi bloqueada", 403);

    // checagem tentativa mudar status
    if (dados.status)
      erro(
        "Tentativa de mudança de status foi bloqueada, use a rota apropriada",
        403,
      );
    ...
    if (typeof senha === "string" && senha.length > 0)
      pessoaAtualizada.senha = await bcrypt.hash(senha, 10);
  ...
  static async ativar(id, requisitante) {
    if (id == requisitante.id)
      erro("Não pode ativar seu próprio cadastro", 403);
    const pessoa = await this.buscarPorId(id, requisitante);
    const permitido = (funcoes[requisitante.funcao]?.gerenciar || []).includes(
      pessoa.funcao,
    );
    if (!permitido) erro("Acesso negado", 403);
  ...
  static async desativar(id, requisitante) {
    if (id == requisitante.id)
      erro("Não pode desativar seu próprio cadastro", 403);
    const pessoa = await this.buscarPorId(id, requisitante);
    const permitido = (funcoes[requisitante.funcao]?.gerenciar || []).includes(
      pessoa.funcao,
    );
